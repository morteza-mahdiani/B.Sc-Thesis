...
cleaner = onto.Axiom()
athletes = cleaneeed.Athlete.unique()
for p in athletes:
    if cleaner.monalityCheck(cleaneeed[cleaneeed['Athlete']== p]) == 'No':
        cleaneeed = cleaner.monalityModify(cleaneeed, p)
        print("detected monalityCheck")
    if cleaner.genderCheck(cleaneeed[cleaneeed['Athlete'] == p]) == 'Yes':
        cleaneeed = cleaner.genderModify(cleaneeed, p)
        print("detected genderCheck")
    for e in ['Wrestling', 'Equestrian', 'Cycling', 'Aquatics']:
        if cleaner.majorCheck(cleaneeed[cleaneeed['Athlete'] == p], e) == 'Yes':
            cleaneeed = cleaner.majorModify(cleaneeed, p)
            print("detected majorCheck")
...
for i,j,k in zip(SVMClean.predict(np.array(YLabel.drop(["Medal"], axis = 1))),
    ...
    print("%s\t%s\t%s" % (i, j, k))
    listSVMClean.append(i)
    listSVMUnclean.append(j)
    listSVMAgainclean.append(k)

# ////////////////////////////////////save data/////////////////////////////////////////////////////////////////////////
l1 = pd.DataFrame(listSVMClean)
l2 = pd.DataFrame(listSVMUnclean)
l3 = pd.DataFrame(listSVMAgainclean)
l1.to_csv("listSVMClean.csv")
l2.to_csv("listSVMUnclean.csv")
l3.to_csv("listSVMAgainclean.csv")
...
pdCandAC.to_csv("rmse_val2_cleanAndAgainClean.csv")
# ////////////////////////////////////Plot raw data/////////////////////////////////////////////////////////////////////
xline = SVMClean.predict(np.array(YLabel.drop(["Medal"], axis = 1)))
yline = SVMAgainclean.predict(np.array(YLabel.drop(["Medal"], axis=1)))
zline = SVMUnclean.predict(np.array(YLabel.drop(["Medal"], axis=1)))
plt.plot(zline, c= 'red', label = 'Unclean Data')
plt.plot(xline, c= 'green', label= 'Original Data')
plt.plot(yline, c= 'blue', label= 'Clean Data')
plt.title('Predicted Values for 3 Data-sets')
plt.xlabel('Number of Label for Prediction')
...
window = tkinter.Tk()
window.title("Phase1 Notification")
tkinter.Label(window, text = "Phase1 completed! find the result in the app directory", fg = "Black").pack()
window.mainloop()